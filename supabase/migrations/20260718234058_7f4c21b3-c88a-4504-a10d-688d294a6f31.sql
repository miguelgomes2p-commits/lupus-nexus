...
BEGIN
  ...
  -- Entradas: MRR dos clientes ativos cujo contrato JÁ INICIOU antes/no fim do período
  -- e cuja data de pagamento (dia do contrato) cai dentro do período
  SELECT COALESCE(SUM(COALESCE(c.monthly_recurring_revenue, 0)), 0)
    INTO v_clients_in
  FROM public.clients c
  WHERE c.status = 'ativo'
    AND c.contract_start_date IS NOT NULL
    AND c.contract_start_date <= v_period_end
    AND (
      (make_date(EXTRACT(YEAR FROM v_period_end)::INT, EXTRACT(MONTH FROM v_period_end)::INT,
        ...
        BETWEEN GREATEST(v_period_start, c.contract_start_date) AND v_period_end)
      OR
...
-- Regenera todos os fechamentos históricos com a lógica corrigida
DO $$
DECLARE
  v_start DATE;
  v_current DATE;
BEGIN
  SELECT LEAST(
    COALESCE((SELECT MIN(contract_start_date) FROM public.clients WHERE status = 'ativo'), CURRENT_DATE),
    COALESCE((SELECT MIN(incurred_at) FROM public.costs), CURRENT_DATE)
  ) INTO v_start;

  v_current := date_trunc('month', v_start)::DATE;
  WHILE v_current <= CURRENT_DATE LOOP
    PERFORM public.generate_monthly_closing(v_current);
